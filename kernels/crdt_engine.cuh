 ...
 */
__device__ __inline__ uint32_t warp_process_command(
    CRDTState* crdt,
    WarpCommand* cmd_ptr
) {
    const int lane_id = threadIdx.x % WARP_SIZE;
 ...
 */
__device__ __inline__ uint32_t warp_parallel_update_32(
    CRDTState* crdt,
    uint32_t base_row,
    uint32_t base_col,
    const double* values,
 ...
 */
__device__ __inline__ uint32_t warp_recalculate_cells(
    CRDTState* crdt,
...
__device__ __inline__ CRDTCell crdt_read_cell(CRDTState* crdt, uint32_t row, uint32_t col)
{
    if (!crdt->is_valid(row, col)) {
        return CRDTCell(); // Return a default/empty cell
    }
    return crdt->get_cell(row, col);
}
 ...
 */
__device__ __inline__ void crdt_coalesced_row_read(
    CRDTState* crdt,
    uint32_t row,
    double* values_out
) {
 ...
 * @param values_out Output array for cell values
 */
__device__ __inline__ void crdt_coalesced_column_read(
    CRDTState* crdt,
    uint32_t col,
    double* values_out
) {
 ...
 */
__global__ __inline__ void crdt_get_metrics_kernel(
    CRDTState* crdt,
...
__device__ uint32_t evaluate_level(
    ...
    if (idx < graph->num_cells && graph->level[idx] == level) {
        ...
        for (uint32_t d = 0; d < cell->num_deps && d < 6; d++) {
            ...
            cell->operands[d] = crdt_read_cell(crdt, dep_row, dep_col).value;
        }
