...
fn simulate_cell_update() -> Duration {
    ...
    let jitter = (rand::random::<f64>() - 0.5) * 3.0;
...
fn print_latency_results(result: &LatencyResult) {
    ...
    println!("  P99.9:     {:8.2} us", result.p999_us);
    println!("  Below target: {} / {} ({:.1}%)",
...
fn generate_optimization_recommendations(result: &LatencyResult) -> Vec<String> {
    ...
    if result.mean_us > TARGET_LATENCY_US as f64 {
        recommendations.push(format!(
            "Average latency {:.2} us exceeds target {} us. Reduce polling interval or use spin-wait.",
            result.mean_us, TARGET_LATENCY_US
        ));
    }

    recommendations.push("Ensure unified memory is pinned for optimal GPU access".to_string());
    recommendations.push("Consider using CUDA streams to overlap memory transfers with computation".to_string());
    recommendations.push("Profile GPU kernel to identify bottlenecks in the critical path".to_string());
...
fn measure_cell_update_latencies() -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    ...
    for _ in 0..WARMUP_ITERATIONS {
        let _latency = Duration::from_micros(5);
    }

    println!("Measuring {} iterations...", TEST_ITERATIONS);
    for i in 0..TEST_ITERATIONS {
        let start = Instant::now();
        let _update_time = simulate_cell_update();
...
mod latency_tests {
    ...
    fn test_cell_update_latency() {
        ...
        assert!(
            result.p95_us < TARGET_LATENCY_US as f64 * 2.0,
            "P95 latency ({:.2} us) exceeds 2x target ({} us)",
            result.p95_us, TARGET_LATENCY_US
        );

        assert!(
            result.below_target > 0,
            "No samples met the target latency of {} us",
            TARGET_LATENCY_US
        );
    }
    ...
    fn test_polling_interval_optimization() {
        ...
        let intervals = vec![0, 1, 10, 100, 1000];
...
mod optimization_tests {
    ...
    fn test_polling_strategies() {
        ...
        for (name, _interval_ns) in strategies {
            println!("\nStrategy: {}", name);
        }
    }
    ...
    fn benchmark_sustained_throughput() {
        ...
        while start.elapsed() < duration {
            ...
            if operations % 1000 == 0 {
                print!("\r  Operations: {} | Avg latency: {:.2} us",
        ...
        println!("Average latency: {:.2} us", total_latency / operations as f64);
    }
}
